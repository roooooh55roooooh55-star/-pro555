...
const LayoutEditor: React.FC<{ initialVideos: Video[] }> = ({ initialVideos }) => {
  ...
  // 3. Duplicate Section (New Feature)
  ...
  return (
    <div className="p-4 sm:p-8 animate-in fade-in duration-500 pb-[800px] min-h-[150vh]">
        ...
        {/* Header Block - Modified Layout */}
        <div className="bg-neutral-900 border border-purple-500/30 p-4 rounded-[2.5rem] shadow-[0_0_30px_rgba(168,85,247,0.1)] mb-4 flex flex-wrap justify-between items-center sticky top-0 z-[60] backdrop-blur-xl bg-neutral-900/80">
            {/* Title Section (Right in RTL) */}
            <div className="flex-1">
                <h1 className="text-xl font-black text-purple-400 mb-1">محرر الواجهة</h1>
                <p className="text-[9px] text-gray-500 uppercase tracking-widest font-bold">DRAG & DROP EDITOR</p>
            </div>

            {/* Action Buttons (Left in RTL - Using justify-end logic or reverse) */}
            <div className="flex items-center gap-2">
                <button 
                  onClick={() => setIsLocked(!isLocked)}
                  className={`flex items-center justify-center w-12 h-12 rounded-2xl font-black text-xs transition-all ${
                    isLocked 
                  ...
                  title={isLocked ? "حرر القفل" : "اقفل التعديل"}
                >
                  {isLocked ? (
                ...
                <button 
                  onClick={saveLayout} 
                  disabled={loading}
                  className="flex items-center justify-center w-12 h-12 bg-purple-600 hover:bg-purple-500 text-white rounded-2xl font-bold text-xs shadow-lg active:scale-95 transition-all"
                  title="حفظ التغييرات"
                >
                  {loading ? (
        ...
        <div className={`sticky top-24 z-50 transition-all duration-500 ${isLocked ? "opacity-50 pointer-events-none grayscale" : "opacity-100"}`}>
        ...
        <div className="space-y-12 relative mt-8">
            {isLocked && (
                <div className="fixed inset-0 top-32 bg-black/80 z-40 backdrop-blur-sm flex items-center justify-center pointer-events-none">
                    <div className="bg-neutral-900 border border-red-500/30 px-8 py-6 rounded-3xl flex flex-col items-center gap-4 shadow-2xl animate-bounce">
                        ...
                        <span className="text-xl font-bold text-white">حرر القفل لتبدأ التعديل</span>
                    </div>
                </div>
            )}
                        ...
                        {section.type === 'shorts_grid' && (
                            <div className="w-full h-full grid grid-cols-2 grid-rows-2 gap-3 p-2">
                                {getPreviewVideos(4, 'Shorts').map(v => {
                                    ...
                                    return (
                                        <div key={v.id} className={`relative rounded-xl overflow-hidden border ${neonStyle} bg-neutral-900`}>
                                            <SafeAutoPlayVideo 
                                                src={formatVideoSource(v)} 
