...
const LongPlayerOverlay: React.FC<LongPlayerOverlayProps> = ({ 
  ...
  // Create an "infinite" suggestions list by shuffling and including self if needed
  const suggestions = useMemo(() => {
      let filtered = allLongVideos.filter(v => v && v.id !== video?.id && v.video_type === 'Long Video');
      // If list runs out, we recycle
      if (filtered.length === 0) {
          filtered = allLongVideos.filter(v => v.video_type === 'Long Video');
      }
  ...
  useEffect(() => {
    ...
    const handleEnd = () => { 
        ...
        } else if (isAutoPlay && allLongVideos.length > 0) {
            // Fallback random
            const random = allLongVideos[Math.floor(Math.random() * allLongVideos.length)];
            onSwitchVideo(random);
        }
    };
  ...
  }, [video?.id, isAutoPlay, suggestions, onSwitchVideo, onProgress, allLongVideos]);
